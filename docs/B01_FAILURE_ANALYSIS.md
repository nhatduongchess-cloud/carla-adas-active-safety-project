...
**Interpretation.** What is *observed* is a pure-virtual call during virtual dispatch on an `FSkeletalMeshSceneProxy`, reached from the **camera sensor's scene-capture** render pass (`UpdateSceneCaptureContent_RenderThread` → `Render_CARLA` → `GatherDynamicMeshElements`) on the RenderThread. A "pure virtual function being called" error typically arises when an object's vtable pointer is the base-class vtable at the moment of the call, which happens while a C++ object is mid-construction or mid-destruction — but it can also follow from a use-after-free or other memory corruption. The evidence here does not identify the object instance, its allocation or its destruction history.

*Corrected 2026-09-25.* This section previously said the stack "directly confirms" an object-lifetime race. That overstated it. The stack is **consistent with** the object-lifetime / concurrent-destruction hypothesis and narrows where to look; it does not confirm it, because no heap history or object identity is available. What it does establish:
...
- The fault is reached through the **camera sensor's `SceneCapture`** path while a scene containing an animated skeletal mesh is captured. That the proxy's lifecycle *races* the capture pass is the leading hypothesis, not an observation.
